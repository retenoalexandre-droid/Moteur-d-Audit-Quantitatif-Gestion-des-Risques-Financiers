...
if __name__ == "__main__":
    ...
    valeur_reelle = CorporateValuation.present_value_dcf(
        future_cash_flow=10_000_000,
        discount_rate=0.10,
        years_in_future=5
    )
    logger.info(f" -> DCF : Valeur actuelle de 10M€ dans 5 ans : {valeur_reelle:,.2f} €")

    trajectoire = CorporateValuation.cash_burn_trajectory(
        initial_cash=50000,
        margin_per_unit=-2.5,
        volume_array=[1000, 5000, 15000, 50000]
    )

    logger.info("\n[VISUALISATION] Génération des graphiques d'analyse...")
    plot_financial_graphics(
        cash_trajectory=trajectoire,
        returns=real_returns,
        var_threshold=resultat_risque['VaR']
    )
